...
Full schemas are returned by `tools/list` and defined in `src/lib/bookings/tools.ts`. Unknown methods and invalid parameters produce JSON-RPC errors. Expected domain failures produce tool results with `isError: true`.
