...
|---|---|---|
| Browser workspace | Conversation, change preview, approval, receipts, lab | `src/components/workspace/workspace.tsx` |
| Intent interpreter | Guided requests or Bedrock Converse to constrained intent | `src/lib/bookings/intent.ts` |
| MCP transport | Initialization, tool discovery, validated tool calls | `src/lib/bookings/mcp.ts` |
| Tool registry | Input schemas and domain dispatch | `src/lib/bookings/tools.ts` |
| Domain engine | Plans, execution, reconciliation, compensation, conflicts | `src/lib/bookings/engine.ts` |
| Persistent store | D1 reads and optimistic atomic updates | `src/lib/bookings/store.ts` |
| Browser action API | Explicit approval, manual edits, lab settings | `src/app/api/action/route.ts` |
...
One aggregate keeps the prototype auditable and deployable. A production marketplace should move shared inventory and reservation identities into normalized tables with service-specific unique constraints, and introduce real external adapters. Provider actions currently share the same database and process.
...
The session cookie scopes all reads and writes. Hosted access is private by default. See [security](security.md) for limitations of the prototype's approval and authentication model.
