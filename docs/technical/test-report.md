...
- `node --experimental-strip-types --test tests/unit/*.test.ts`: **47 passed, 0 failed** after release finalization on September 15, 2026.
- `node node_modules/typescript/bin/tsc --noEmit`: passed for the release source.
- Production Worker/client build: passed.
- `node tests/integration/worker.mjs`: passed with a disposable real D1 database in Miniflare.
